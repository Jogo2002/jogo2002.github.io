...
```{r simulate-type2}
simulate_type2_error <- function(n = 30, reps = 20, perms = 100, true_diff = 0.2, seed = 47) {
  set.seed(seed) 
...
simulate_type2_error(n = 30, reps = 20, perms = 100, true_diff = 0.2)
```

**Interpretation:**\
This number tells us that the permutation test **fails to reject H₀** 80% of the time when there is a true difference in volatility (with a true difference of 0.2 in standard deviations).\

An 80% Type II error rate indicates that the test has low power under these conditions. This could be due to factors such as:
